<x-modal>
    ...
    <x-slot name="content">
        <form wire:submit="update" id="editCategory" class="space-y-3 md:space-y-3">
            <x-input-text wire:model.live="name" name="name" id="name" label="Category Name" value="{{ $category->name }}" required placeholder="Input Name"/>
        </form>
    </x-slot>

    <x-slot name="buttons">
        <button type="submit" form="editCategory"
